  ...
  return (
    <dialog id="viewModal" className="modal">
      <div className="modal-box max-w-5xl">
        ...
        <table className="w-full border-collapse rounded mt-8">
          <thead className="border bg-black">
            <tr>
              <th className="border-r border-white font-semibold">Criteria</th>
              {currentAssessment?.scoringScale?.map((level) => (
                <th key={level._id} className="text-sm border-r border-white">
                  {level.score} - {level.description}
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="border border-white text-white">
