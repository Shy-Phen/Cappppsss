...
const EvaluatedCard = ({ evalu }) => {
  ...
  return (
    <div className="rounded-lg p-6  shadow-lg hover:shadow-xl transition-shadow duration-300 hover:scale-105 transform bg-primary text-primary-content">
      <h2 className="text-sm font-semibold mb-2">
        {evalu.title[0].toUpperCase() + evalu.title.slice(1)}
      </h2>

      <div className="grid grid-cols-2 h-8 mb-4">
        {evalu?.member?.map((mem, i) => (
